from api.agents.advanced.nodes import coordinator_agent
from api.agents.common.models import StateAdvanced as State
from api.core.config import OPENAI, MODELS
...
results = ls_client.evaluate(
    lambda x: coordinator_agent(
        State(messages=x["messages"]),
        provider=OPENAI,
        model_name=MODELS[OPENAI][0],
    ),
...
feedback_stats_exist = (
    results_resp.feedback_stats.get("next_agent_evaluator") is not None
)
